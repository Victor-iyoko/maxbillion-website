...
function Button({link="", text, dark, isLoading}) {
  const classes = `relative overflow-hidden flex items-center justify-center tracking-wider 
  ...
  return (
          <>
              {
                link !== "" ?  ( <a className={classes} href={link}>{text}</a> ) :
                (
                  <button disabled={isLoading ? true : false} className={classes}>
